...
> Weitere Informationen finden Sie unter: [Kompilieren von DSC-Konfigurationen / Anmeldeinformationsobjekte](/azure/automation/automation-dsc-compile#credential-assets)
...
Dies ist ein Auszug aus der "MOF"-Datei, die von der Konfiguration für "TestMachine1" generiert. Die `System.Security.SecureString` in verwendet die Konfiguration wurde in nur-Text konvertiert und gespeichert, in der Datei "MOF" als eine `MSF_Credential`. Ein `SecureString` ist mit dem aktuellen Benutzerprofil verschlüsselt. Dies funktioniert gut mit allen Formen der PowerShell-Remoteverwaltung. Eine "MOF"-Datei fungiert als ein eigenständigen allein-Konfigurationsverfahren. Ab in PowerShell 5.0 ist werden "MOF"-Dateien auf einem Knoten verschlüsselt, im Ruhezustand, aber nicht in der Übertragung an den Knoten. Dies bedeutet, dass Kennwörter in einem "MOF"-Datei als Klartext verfügbar gemacht werden, wenn Sie diese auf einen Knoten anwenden. Um Anmeldeinformationen zu verschlüsseln, müssen Sie eine **Pull-Server**. Weitere Informationen finden Sie unter [Sichern von MOF-Dateien mit Zertifikaten](../pull-server/secureMOF.md).
...
## <a name="example-the-group-resource-credential-property"></a>Beispiel: Die Ressource "Group" Credential-Eigenschaft
