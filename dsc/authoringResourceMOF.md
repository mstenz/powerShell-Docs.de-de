...
Um eine benutzerdefinierte DSC-Ressource mit einem MOF-Schema zu implementieren, erstellen Sie die folgende Ordnerstruktur. Das MOF-Schema wird in der Datei „Demo_IISWebsite.schema.mof“ definiert, und das Ressourcenskript in „Demo_IISWebsite.ps1“. Optional können Sie eine Modulmanifestdatei (psd1) erstellen.
...
* Der Typqualifizierer `[Key]` für eine Eigenschaft gibt an, dass die Ressourceninstanz durch diese Eigenschaft eindeutig identifiziert wird. Eine `[Key]`-Eigenschaft ist ebenfalls erforderlich.
...
* Eine Eigenschaft namens `Ensure` zu Ihrer Ressource hinzuzufügen, wird als Möglichkeit zum Beibehalten eines konsistenten Stils mit integrierten DSC-Ressourcen empfohlen.
