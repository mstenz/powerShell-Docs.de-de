...
Sie definieren die Ressourcen, die Ihren Management OData-Webdienst verfügbar macht, durch das Erstellen einer MOF-Datei, die das Schema für die öffentliche Ressourcengruppe verwendet. Jede Ressource wird als Klasse in der Datei definiert, und Eigenschaften werden als Klassenmember definiert. Weitere Informationen zu Schemas, die in der MOF-Datei verwendet, finden Sie unter [öffentliche Ressourcengruppe Schema](./public-resource-schema.md).

## <a name="example-mof-file"></a>Beispiel-MOF-Datei

Die folgende Datei definiert, Dienst und Prozessressourcen. Jeder dieser Ressourcen entspricht ein Objekt, das durch eine Reihe von Windows PowerShell-Cmdlets verwaltet werden kann. Die Eigenschaften entsprechen Parametern, die von diesen Cmdlets verwendet.

Jede der beiden Ressourcen enthält Eigenschaften, die von einem komplexen Typ sind. Die komplexeren Typen definiert sind, entsprechend der Änderung von Klassen mit dem `ComplexType` Qualifizierer.
...
[Öffentliche Ressourcengruppe-Schema](./public-resource-schema.md)