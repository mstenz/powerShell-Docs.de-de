...
Gibt an, ob die Größe der Spalte und die Anzahl der Spalten basierend auf der Größe der Daten angepasst werden.

-Element (Format) ViewDefinitions-Element (Format) anzeigen (Format)-Element WideControl-Element (Format) Autosize Konfigurationselement für WideControl (Format)
...
Den folgenden Abschnitten werden Attribute, untergeordnete Elemente und das übergeordnete Element des der `AutoSize` Element.
...
Wenn Sie eine Breite Ansicht definieren zu können, können Sie Hinzufügen der `AutoSize` Element oder die [ColumnNumber](./columnnumber-element-for-widecontrol-format.md) -Element, aber Sie können nicht beide hinzufügen.
