...
In den folgenden Abschnitten werden die Attribute, die untergeordneten Elemente und das über `TableColumnItem` geordnete Element des-Elements beschrieben.
...
Dieses Beispiel zeigt ein `TableColumnItem` -Element, das den Wert `Status` der-Eigenschaft des [System. Diagnostics. Process](/dotnet/api/System.Diagnostics.Process) -Objekts anzeigt.
