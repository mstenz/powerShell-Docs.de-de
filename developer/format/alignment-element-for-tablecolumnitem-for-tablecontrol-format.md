...
Element (Format) ViewDefinitions-Element (Format) anzeigen (Format)-Element TableControl-Element (Format) TableRowEntries-Element (Format) TableRowEntry-Element (Format) TableColumnItems-Element (Format) TableColumnItem Konfigurationselement (Format) Alignment-Element für TableColumnItem (Format)
...
Geben Sie einen der folgenden Werte. (Diese Werte sind keine Groß-/Kleinschreibung berücksichtigt.)

Verschiebungen nach links die Daten in der Spalte auf der linken Seite angezeigt. (Dies ist die Standardeinstellung, wenn dieses Element nicht angegeben ist.)

Verschiebungen nach rechts die Daten in der Spalte rechts angezeigt.
