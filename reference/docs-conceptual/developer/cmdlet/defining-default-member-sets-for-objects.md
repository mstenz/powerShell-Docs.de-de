...
Die folgende Ausgabe zeigt die Standardeigenschaften, die vom Cmdlet " [Format-List](/powershell/module/Microsoft.PowerShell.Utility/Format-List) " zurückgegeben werden. Nur die Eigenschaften "`Id`", "`Handles`", "`CPU`" und "`Name`" werden für jedes Prozess Objekt zurückgegeben.
