...
Wenn das Cmdlet bei Windows PowerShell registriert wurde, testen Sie es, indem Sie es in der Befehlszeile ausführen. Im folgenden finden Sie einige Tests, die zeigen, wie die Parameter "`ProcessId`" und "`InputObject`" zum Testen Ihrer Parametersätze verwendet werden können, um einen Prozess zu verhindern.

- Führen Sie in Windows PowerShell das Cmdlet "Break-proc" mit dem Parameter "`ProcessId`" aus, um einen Prozess auf der Grundlage des Bezeichners zu unterbinden. In diesem Fall verwendet das Cmdlet den `ProcessId`-Parametersatz, um den Prozess zu verhindern.
...
- Führen Sie in Windows PowerShell das Cmdlet "Break-proc" mit dem Parameter "`InputObject`" aus, um Prozesse für das vom `Get-Process`-Befehl abgerufene Notepad-Objekt zu verhindern.
